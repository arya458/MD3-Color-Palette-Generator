...
interface PaletteCard extends HTMLDivElement {
  paletteData: Palette;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let currentActiveCard: PaletteCard | null = null;
  ...
  const renderPatternCard = (palette: Palette): void => {
    const card = document.createElement('div') as PaletteCard;
...
`;
  ...
  patternList.addEventListener('click', (e: MouseEvent) => {
    const card = (e.target as HTMLElement).closest('.pattern-card') as PaletteCard;
    if (!card) return;

    const actionBtn = (e.target as HTMLElement).closest('.action-btn') as HTMLElement;
    if (actionBtn) {
      e.stopPropagation();
  ...
  const generateInitialPatterns = async (count = 20): Promise<void> => {
    ...
    const firstCard = patternList.querySelector('.pattern-card') as PaletteCard;
